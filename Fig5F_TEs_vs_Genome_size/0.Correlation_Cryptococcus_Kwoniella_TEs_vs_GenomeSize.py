...
Script Metadata
--------------------------------------------------------------------------------
Title: Correlation Analysis of Genome Size and Transposable Element (TE) Percentage
Author: Marco A. Coelho @ Heitman lab, Duke University
Date: 2023-05-08
Description: This script performs a correlation analysis between genome size and
...
# Extract and convert genome size to Megabases (Mb)
genome_size = data[['Organism', 'gen']].drop_duplicates()
genome_size['gen_Mb'] = genome_size['gen'] / 1e6

# Merge TE percentage data with genome size information
merged_data = pd.merge(total_percentage, genome_size, on='Organism')
...
_, p_value_gen = stats.shapiro(merged_data['gen_Mb'])

# Correlation Analysis (deciding on Pearson or Spearman based on normality test results)
if p_value_te < 0.05 or p_value_gen < 0.05:
    corr_coef, p_value = stats.spearmanr(merged_data['gen_Mb'], merged_data['percentage'])
    method_used = "Spearman's rho"
else:
    corr_coef, p_value = stats.pearsonr(merged_data['gen_Mb'], merged_data['percentage'])
    method_used = "Pearson's r"

# Visualization
#--------------------------------------------------------------------------------
plt.figure(figsize=(6, 6))
sns.scatterplot(x='gen_Mb', y='percentage', hue='Group', data=merged_data,
                palette={'Cryptococcus': '#f7a38f', 'Kwoniella': '#9bd8f2'})
plt.xlabel('Genome Size (Mb)')
plt.ylabel('Percentage of TEs')
plt.title('Correlation between Genome Size and TE Percentage')
plt.text(x=np.min(merged_data['gen_Mb']), y=np.max(merged_data['percentage']),
         s=f'{method_used}: {corr_coef:.2f}, p-value: {p_value:.3f}',
         horizontalalignment='left', verticalalignment='top')

plt.rcParams['pdf.fonttype'] = 42
plt.savefig('TEs_vs_GenomeSize_Correlation_with_Coefficient.pdf')
plt.show()
...
stats_results = {
    ...
    'Normality Shapiro-Wilk test p-value (genome size in Mb)': p_value_gen
}
stats_df = pd.DataFrame([stats_results])
stats_df.to_csv('TEs_vs_GenomeSize_Correlation_Stats.csv', index=False)
...
merged_data_sorted.to_csv('TEs_vs_GenomeSize_MergedData_Sorted.csv', index=False)

# Display file paths of saved files
stats_file_path = 'TEs_vs_GenomeSize_Correlation_Stats.csv'
merged_data_file_path = 'TEs_vs_GenomeSize_MergedData_Sorted.csv'
